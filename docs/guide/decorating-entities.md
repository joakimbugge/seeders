...
When seeding an entity with relations, those related entities are seeded too. `@Seed()` on a relation property **always creates a new entity instance** — it does not select an existing one from the database. If you need to link to a specific existing entity, use the [`values` option](/guide/seeding-entities#overriding-seeded-values).
